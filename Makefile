...
test-backend: ## Run tests for backend services
	...
	@echo -e "${YELLOW}Tidying auth-svc modules (if needed)...${NC}"
	@cd $(BACKEND_DIR)/auth-svc && go mod tidy && cd ../..
	@echo -e "${YELLOW}Proceeding with tests...${NC}"
	@for svc in auth-svc user-svc curriculum-svc progress-svc assessment-svc tutor-orchestrator-svc notification-svc api-gateway; do \
		echo -e "${YELLOW}Testing $${svc}...${NC}"; \
		if [ -d "$(BACKEND_DIR)/$${svc}" ]; then \
			# Run tests in a subshell to isolate directory changes \
			(cd $(BACKEND_DIR)/$${svc} && go test ./... -v && cd /app) ; \
		else \
			echo -e "${RED}Error: Service directory $(BACKEND_DIR)/$${svc} not found.${NC}"; \
		fi; \
	done
